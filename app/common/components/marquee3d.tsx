...
export function Marquee3D({ images }: { images: string[] }) {
  ...
  // 만약 images가 비어있으면 기본 reviews 사용
  const reviewsToUse =
  ...
  const secondRow = reviewsToUse.slice(
    Math.ceil(reviewsToUse.length / 4),
    Math.ceil(reviewsToUse.length / 2)
  );
  const thirdRow = reviewsToUse.slice(
    Math.ceil(reviewsToUse.length / 2),
    Math.ceil((reviewsToUse.length * 3) / 4)
  );
  const fourthRow = reviewsToUse.slice(
    Math.ceil((reviewsToUse.length * 3) / 4)
  );
      ...
      >
        <Marquee pauseOnHover vertical className="[--duration:20s]">
          {firstRow.map((review, index) => (
            <ReviewCard key={`first-${index}-${review.img}`} {...review} />
          ))}
        </Marquee>
        <Marquee reverse pauseOnHover className="[--duration:20s]" vertical>
          {secondRow.map((review, index) => (
            <ReviewCard key={`second-${index}-${review.img}`} {...review} />
          ))}
        </Marquee>
        <Marquee reverse pauseOnHover className="[--duration:20s]" vertical>
          {thirdRow.map((review, index) => (
            <ReviewCard key={`third-${index}-${review.img}`} {...review} />
          ))}
        </Marquee>
        <Marquee pauseOnHover className="[--duration:20s]" vertical>
          {fourthRow.map((review, index) => (
            <ReviewCard key={`fourth-${index}-${review.img}`} {...review} />
          ))}
        </Marquee>
      </div>
